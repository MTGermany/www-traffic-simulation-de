...
public class SimCanvas extends Canvas implements Runnable, Constants{
	...
	diaWidth=(isRingroad) 
	    ? xsize-xc-r2-2*padding
	    : (int)(0.5*(xsize-xc))-3*padding;
...
public void run(){
    ...
    if (showInsetdiagrams){setInsetDiagrams();}
   ...
   // sets runner=null -> run() method ends
	...
	// simulate new timestep
	    ...
	    if (showInsetdiagrams){updateInsetDiagrams();}
        }
...
private void paintSim(Graphics g){
	...
	if (true) {
	    ...
	    if (choice_Geom==0){updateCircle(g);}
	    if (choice_Geom==1){updateU(g);}
	  ...
	  // paint also the vehicles; therefore also the updte.. calls
	    if (choice_Geom==0){paintCircle(g);updateCircle(g);}
	    if (choice_Geom==1){paintU(g);updateU(g);}
	}
	...
	if(VEH_COLORCODE==1){
	    ...
	    else{paintColormapSpeed(g,0.5,0.2,0.2,0.10);}
	}
	if (showInsetdiagrams){paintInsetDiagrams(g);}
	if (((choice_Szen==3)||(choice_Szen==4)||(choice_Szen==5))
	...
	boolean showInsetdiagrams=SHOW_INSET_DIAG && (choice_Szen==1);
	    ...
	    PolyVehCircle pvc= new PolyVehCircle(pos[i],lane[i],
						 lengthVeh[i], VEH_WIDTH_M, 
						 r1, r2, xc, yc);

	    int car_xPoints[] = pvc.xPoints;
	    int car_yPoints[] = pvc.yPoints;
            if ((showInsetdiagrams)&&(nr[i]==fcnr)){ g.setColor(Color.blue); }
            else{ g.setColor((Color)(colors.elementAt(i)));}
	    g.fillPolygon(car_xPoints, car_yPoints, 4);

 	    //martin
	    Color vehColor=(showInsetdiagrams&&(nr[i]==fcnr))
	    ...
	    if(nr[i]==fcnr){vehimg.setProbeveh();}
	    if(nr[i]==fcnr){vehimg.setProbeveh();}
	...
	for (int i=imax; i>=0; i--){
	  ...
	  vehimg.draw(g,1);
        ...
        if(choice_Szen==2){
	      ...
	      int xc_veh=xc +lRamp
		  + (int)(scale*(0.5*lengthVehOn[i]+posOn[i] - onramp.roadLength ));
	      int yc_veh=yc+r2+(int)(0.25*lw+scale*VEH_WIDTH_M);
	      VehicleImage2D vehimg=new VehicleImage2D(lengthVehOn[i],VEH_WIDTH_M,
						       vehType, false, vehColor, scale);
...
0.075,"75 Veh/km",0.417,"1500 Veh/h");
    ...
    */
    ...
    private void paintColormapSpeed(Graphics g, double xrelCenter, double yrelCenter, 
				    double widthRel, double heightRel){
