...
private int choice_Scen=CHOICE_SCEN_INIT;
    ...
    private double qIn=(CHOICE_SCEN_INIT==2)
?Q_INIT2/3600. : (CHOICE_SCEN_INIT==3)
...
?Q_INIT4/3600. : Q_INIT5/3600.;
    ...
    // when changing SIM_BG_COLOR, also change it in de/trafficsimulation/SimCanvas.java
    ...
    private Label label_simSpeed1, label_simSpeed2,
    ...
    private Label label_IDMv0
        = new Label(String.valueOf (V0_INIT_KMH)+" km/h");
    //private Label label_IDMv0
    // = new Label("TestTestTestTest");
    private Label label_IDMT
        = new Label(String.valueOf (T_INIT_S)+" s");
    private Label label_IDMa
        = new Label(String.valueOf (A_INIT_CAR_MSII)+" m/s^2");
    private Label label_IDMb
        = new Label(String.valueOf (B_INIT_MSII)+" m/s^2");
    private Label label_IDMs0
        = new Label(String.valueOf (S0_INIT_M)+" m");
    private Label label_IDMs1
        = new Label(String.valueOf (S1_INIT_M)+" m");
    ...
    private String str_start_button;
    private String str_stop_button;
    ...
    private Scrollbar sb_IDMv0
       = AdjustmentMethods.getSB(V0_MIN_KMH,V0_MAX_KMH,V0_INIT_KMH, false);
    private Scrollbar sb_IDMT
       = AdjustmentMethods.getSB(T_MIN_S,T_MAX_S,T_INIT_S, false);
    private Scrollbar sb_IDMa
       = AdjustmentMethods.getSB(A_MIN_MSII,A_MAX_MSII,A_INIT_CAR_MSII, false);
    private Scrollbar sb_IDMb
       = AdjustmentMethods.getSB(B_MIN_MSII,B_MAX_MSII,B_INIT_MSII, false);
    private Scrollbar sb_IDMs0
       = AdjustmentMethods.getSB(S0_MIN_M,S0_MAX_M,S0_INIT_M, false);
    private Scrollbar sb_IDMs1
       = AdjustmentMethods.getSB(S1_MIN_M,S1_MAX_M,S1_INIT_M, false);
    ...
    private SimCanvas simCanvas; //=new SimCanvas (2, density, qIn, perTr, p_factor, deltaB, p_factorRamp, deltaBRamp, tsleep_ms);// simulation field
    ...
    public void init(){
     ...
     // set default scenario: Default CHOICE_SCEN_INIT; overridden by cmd line in html
     ...
     switch(userScenario){
     case 1 : choice_Scen=1; break; // ringroad
     case 2 : choice_Scen=2; break; // onramp
     case 3 : choice_Scen=3; break; // laneclosing
     case 4 : choice_Scen=4; break; // uphill
     case 5 : choice_Scen=5; break; // traffic lights
     default : System.out.println("parameter: scenario index "+userScenario +" is not implemented ... ");
     }
     }
     ...
     simCanvas=new SimCanvas (choice_Scen, density,
     qIn, perTr, p_factor, deltaB,
     p_factorRamp, deltaBRamp, tsleep_ms);// simulation field
    
if(choice_Scen!=2){
              simCanvas.newValues(choice_Scen, density, qIn, perTr,
				  v0_limit, p_factor, deltaB, tsleep_ms);}
else{
...
else if(CONTROL_SIZE==2){setSize (APPL_WIDTH,APPL_HEIGHT);}
setFonts();
//############################################
...
start_button=new Button(str_start_button);
...
sb_perTr2 = getSB(0, 100, truckPerc);
...
pRamp.add(new Label(str_trucks), gbconstr);
...
pClosedSystem.add(label_density = new Label(
           String.valueOf(dens_invkm)+str_vehperkm),gbconstr);
pRamp.add(label_qIn2 = new Label(
            String.valueOf (flow_invh) + str_vehperh),gbconstr);
pSource.add(label_qIn3 = new Label(
            String.valueOf (flow_invh) + str_vehperh),gbconstr);
pLanechange.add(label_p_factor6 = new Label(
            String.valueOf(p_factor)), gbconstr);

gbconstr.gridx = 2;
gbconstr.gridy = 1;

pClosedSystem.add(label_perTr1 = new Label(
            String.valueOf((int)(100*FRAC_TRUCK_INIT_CIRCLE)) +" %"),gbconstr);
pRamp.add(label_qRamp = new Label(
            String.valueOf (flowRamp_invh) + str_vehperh),gbconstr);
pSource.add(label_perTr35 = new Label(
            String.valueOf(truckPerc) +" %"),gbconstr);
pLanechange.add(label_deltaB6 = new Label(
            String.valueOf(deltaB)+" m/s^2"), gbconstr);

gbconstr.gridx = 2;
gbconstr.gridy = 2;
pClosedSystem.add(label_simSpeed1 = new Label(
            str_warpfactor +str_framerate),gbconstr);
pRamp.add(label_perTr2 = new Label(
            String.valueOf(truckPerc) +" %"),gbconstr);
pSource.add(label_v0_limit = new Label(
            String.valueOf((int)(3.6*v0_limit)) +" km/h"), gbconstr);
pLanechange.add(label_simSpeed6 = new Label(
            str_warpfactor +str_framerate), gbconstr);
...
pSource.add(label_p_factor35 = new Label(
             String.valueOf(p_factor)), gbconstr);
...
pSource.add(label_simSpeed3 = new Label(
                str_warpfactor +str_framerate), gbconstr);
...
sb_IDMs0,str_IDMs0,label_IDMs0);
...
// not clear why I must subtract addtl. getInsets().top below
        ...
        int sbWidth = isCircular
            ? (int)(0.70*simSize) // weiter unten auch veraendern!
            : (int)(0.7*usedWidth);
        int sbHeight = isCircular
            ? (int)(0.16*simWinHeight)
...
: (int)(0.95*usedWidth - sbWidth);
        int sbTop = isCircular
            ? simWinTop + (int)(0.5*(simSize - 1.2*sbHeight-sb_IDMheight))
            : simWinTop + (int)(0.5*(simSize - 1.2*sbHeight-sb_IDMheight))
                        - (int)(0.2*sbHeight);
        ...
        pScrollbars.setBounds(sbLeft,sbTop,
                                sbWidth,sbHeight);
        pIDMparams.setBounds(sb_IDMleft,sb_IDMtop,
                                sb_IDMwidth,sb_IDMheight);
...
else if (evt.target == sb_perTr2) {
  int newval = sb_perTr2.getValue();
  double pT = newval/100.0;
  if (pT != perTr) {
    perTr = pT;
    label_perTr2.setText (String.valueOf (newval)+" %");
  }
}
...
if (event.target==button1){
  ...
  simCanvas.newValues(choice_Scen, density,
...
if (event.target==button2){
  ...
  sb_perTr2.setValue(truckPerc2);
  label_perTr2.setText (String.valueOf (truckPerc2)+" %");
  ...
  simCanvas.newValues2(
                     qIn, perTr, p_factor, deltaB, qRamp, p_factorRamp,
...
simCanvas.newValues2(qIn, perTr, p_factor, deltaB, qRamp,
                                   p_factorRamp, deltaBRamp, tsleep_ms);
}
       makeGlobalLayout();
       if(true){System.out.println("MicroSim.start()");}
       if(false){
           System.out.print("start() => before simCanvas.start(choice_Szen=");
           System.out.println(choice_Scen+", density="+density+")");
       }
       MicroSim.this.appletResized();// (jul11) DOS
       //appletResized(); //DOS
    ...
    private void setLanguage(){
     ...
     str_start_button = lang.getStartName();
     str_stop_button = lang.getStopName();
